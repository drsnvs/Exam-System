<%-- 
    ...
    Created on : 3 Apr, 2024, 10:14:44 PM
    Author     : DARSHAN
--%>
...
//                }else{
                    ps = con.prepareStatement("delete from result where username=?;");
...
//                }
                ...
                if (rowsAffected > 0) {
                    out.println("<script>alert('Result deleted successfully');"
                            + "window.location.href = 'Studentresults.jsp';</script>");
                    // Redirect back to registeredStudents.jsp after deletion
                } else {
                    out.println("<script>alert('Failed 1 to delete Result');</script>");
                }
            } catch (Exception e) {
                out.println("<script>alert('Failed 2 to delete Result');</script>");
                e.printStackTrace();
            } finally {
